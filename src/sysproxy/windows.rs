...
impl Sysproxy {
    pub fn get_system_proxy() -> Result<Sysproxy> {
        ...
        // ProxyServer can be "host:port", "scheme=host:port;...", or "scheme://host:port".
        // We strip any scheme prefix and take the first entry (before ';').
        let (host, port) = if server.is_empty() {
            ("".into(), 0)
        } else {
            // Take the first entry when multiple protocols are listed (e.g. "http=h:p;https=h:p")
            let first = server.split(';').next().unwrap_or(server);
            // Strip "scheme=" or "scheme://" prefix
            let addr = if let Some(eq_pos) = first.find('=') {
                &first[eq_pos + 1..]
            } else {
                first
            };
            ...
            match addr.rsplit_once(':') {
                ...
                None => return Err(Error::ParseStr(server.to_string())),
            }
        };
    ...
    pub fn set_system_proxy(&self) -> Result<()> {
        // Persist enable/disable state to the registry so that apps reading
        // the registry directly (and across reboots) see the correct value.
        let hkcu = RegKey::predef(enums::HKEY_CURRENT_USER);
        let cur_var = hkcu.open_subkey_with_flags(SUB_KEY, enums::KEY_SET_VALUE)?;
        cur_var.set_value("ProxyEnable", &(self.enable as u32))?;
...
impl Autoproxy {
    pub fn get_auto_proxy() -> Result<Autoproxy> {
        ...
        let url = cur_var
            ...
            .unwrap_or_default();
        // Only consider auto-proxy enabled when the URL is non-empty.
